...
ComputeHDI <- function(data, credible.region) {
  data <- sort(data)
  ci.interval <- ceiling(credible.region * length(data))
  length.ci <- length(data) - ci.interval
  ci.mass <- lapply(1:length.ci, function(i) data[i + ci.interval] - data[i])
  
  hdi.min <- data[which.min(ci.mass)]
  hdi.max <- data[which.min(ci.mass) + ci.interval]
  hdi <- c(hdi.min, hdi.max)
